...
\vskip4pt
\begin{itemize}\setlength{\itemsep}{2pt}
  \item We started using Claude Code and Codex in December while working on the Economics AI letter
  \item The learning curve is real, but it is short --- a few days of real use, not a semester
\end{itemize}
\end{tcolorbox}
\end{column}
\begin{column}{0.47\textwidth}
\begin{tcolorbox}[keybox, title={\faRocket\quad Early Surprises}]
\scriptsize
\begin{itemize}\setlength{\itemsep}{2pt}
  \item Extracted due dates from a syllabus into a CSV --- two minutes instead of twenty
  \item Built a full course website in an afternoon
  \item It confidently cited a paper that does not exist
\end{itemize}
\end{tcolorbox}
\end{column}
\end{columns}
\vskip6pt
{\scriptsize\color{CoolGray} The learning curve is real, but it is short --- and the mistakes are educational.}
\end{frame}
...
%% ====== SLIDE 22: EMILY — VOICE FILE COMPARISON ======
