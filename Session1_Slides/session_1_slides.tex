...
\begin{itemize}\setlength{\itemsep}{1pt}
  \item \textbf{Claude} (claude.ai) $\rightarrow$ Claude Code
  \item \textbf{ChatGPT} (chatgpt.com) $\rightarrow$ Codex
\end{itemize}
\vskip2pt
CLI requires a paid subscription (\$20/mo), but any level works
\end{tcolorbox}
\vskip3pt
\begin{tcolorbox}[keybox, title={\faTerminal\quad Step 2: Install}]
\small
Open Terminal (Mac) or PowerShell (Win).\\[4pt]
\textbf{Claude Code:}\\[2pt]
\texttt{\scriptsize curl -fsSL https://claude.ai/install.sh | bash}\\[2pt]
{\scriptsize (Windows: \texttt{irm https://claude.ai/install.ps1 | iex})}\\[4pt]
\textbf{Codex:} download binary from\\[1pt]
{\scriptsize\texttt{github.com/openai/codex/releases}}
\end{tcolorbox}
\end{column}
\begin{column}{0.47\textwidth}
\begin{tcolorbox}[keybox, title={\faPlay\quad Step 3: Launch}]
\small
Close and reopen your terminal, then type \texttt{claude} or \texttt{codex} and follow the sign-in prompts.
\end{tcolorbox}
\vskip3pt
\begin{tcolorbox}[keybox, title={\faExclamationTriangle\quad Windows note}]
\small
Claude Code requires \textbf{Git for Windows} --- install it from \texttt{git-scm.com} first. And use \textbf{PowerShell}, not Command Prompt.
\end{tcolorbox}
\vskip3pt
\begin{tcolorbox}[accentbox, top=2pt, bottom=2pt]
\scriptsize
\textbf{Full guide with troubleshooting:}\\
\texttt{thinkingwithagents.github.io/prep.html}
\end{tcolorbox}
\end{column}
\end{columns}
\end{frame}
...
%% ====== SLIDE 16b: PERMISSIONS SPECTRUM (4 overlays in one frame) ======
...
\begin{tikzpicture}[x=0.09\textwidth]
  ...
  \node[below, font=\small\bfseries, text=SoftCoral] at (10,-0.05) {Clawdbot};
  ...
  \draw[AccentBlue, thick, decorate, decoration={brace, amplitude=3pt, mirror}]
    (1.5,-0.9) -- (6.0,-0.9);
  \node[below, font=\scriptsize\color{AccentBlue}] at (3.75,-1.05)
...
% Overlay 2: Sandbox
\only<2>{%
\begin{tcolorbox}[colback=PaleSage, colframe=SlateTeal!30, boxrule=0.4pt, left=8pt, right=8pt, top=5pt, bottom=5pt]
\small
\textbf{Sandbox (left end)}
\vskip3pt
\begin{itemize}\setlength{\itemsep}{1pt}
...
% Overlay 3: GOD MODE
\only<3>{%
\begin{tcolorbox}[colback=SoftCoral!6, colframe=SoftCoral!50, boxrule=0.4pt, left=8pt, right=8pt, top=5pt, bottom=5pt]
\small
\textbf{GOD MODE (right end)}
\vskip3pt
\begin{itemize}\setlength{\itemsep}{1pt}
...
% Overlay 4: Screenshots
\only<4>{%
\begin{columns}[c]
\begin{column}{0.32\textwidth}
\centering
\includegraphics[width=\textwidth, height=4.5cm, keepaspectratio]{figures/clawbot-1.jpg}
\end{column}
\begin{column}{0.32\textwidth}
\centering
\includegraphics[width=\textwidth, height=4.5cm, keepaspectratio]{figures/clawbot-2.jpg}
\end{column}
\begin{column}{0.32\textwidth}
\centering
\includegraphics[width=\textwidth, height=4.5cm, keepaspectratio]{figures/clawbot-3.jpg}
\end{column}
\end{columns}
\vskip2pt
{\tiny\color{SoftCoral} Meta AI security and safety researcher.}
}%
...
\begin{column}{0.47\textwidth}
\vskip2pt
\includegraphics[width=\textwidth, height=0.65\textheight, keepaspectratio]{blattman-tweet.png}
\end{column}
\end{columns}
\end{frame}
...
\begin{tcolorbox}[colback=DarkSlate, colframe=DarkSlate, arc=3pt, left=8pt, right=8pt, top=3pt, bottom=3pt]
{\color{CoolGray}\ttfamily\tiny /frontend-design\\[2pt]
\color{white}\tiny
I have a website at erkmengirayaslim.com. Please redesign it with a modern, clean aesthetic. Use anthropic.com for design inspiration (minimal, spacious) and caitlinmyers.github.io for academic content structure.\\[2pt]
Create three distinct mock-ups:\\
1. Minimal dark theme with accent colors\\
2. Light academic theme with card-based layout\\
3. Bold modern theme with full-width hero sections\\[2pt]
Download all content from my current site. Save HTML files and screenshot PNGs to the current directory.\\[2pt]
Focus on: clean typography, clear navigation, prominent research section, responsive design.
}
\end{tcolorbox}
\vskip3pt
\begin{tcolorbox}[quotebox, width=12cm, top=2pt, bottom=2pt]
