...
%% ====== SLIDE 7b: WHAT IT ACTUALLY COSTS ======
\begin{frame}{What It Actually Costs --- Real Usage}
\begin{columns}[T]
\begin{column}{0.32\textwidth}
\begin{tcolorbox}[colback=PaleSage, colframe=CoolGray, boxrule=0.5pt, arc=3pt, left=2pt, right=2pt, top=2pt, bottom=2pt, title={\tiny Spending Overview}, fonttitle=\bfseries\tiny, coltitle=DeepCharcoal, colbacktitle=PaleSage!80]
\centering
\includegraphics[width=\textwidth]{claude-spend-overview.png}
\end{tcolorbox}
\end{column}
\begin{column}{0.32\textwidth}
\begin{tcolorbox}[colback=PaleSage, colframe=CoolGray, boxrule=0.5pt, arc=3pt, left=2pt, right=2pt, top=2pt, bottom=2pt, title={\tiny Usage by Model}, fonttitle=\bfseries\tiny, coltitle=DeepCharcoal, colbacktitle=PaleSage!80]
\centering
\includegraphics[width=\textwidth]{claude-usage-panel.png}
\end{tcolorbox}
\end{column}
\begin{column}{0.32\textwidth}
\begin{tcolorbox}[colback=PaleSage, colframe=CoolGray, boxrule=0.5pt, arc=3pt, left=2pt, right=2pt, top=2pt, bottom=2pt, title={\tiny Cost Breakdown}, fonttitle=\bfseries\tiny, coltitle=DeepCharcoal, colbacktitle=PaleSage!80]
\centering
\includegraphics[width=\textwidth]{claude-usage-table.png}
\end{tcolorbox}
\end{column}
\end{columns}
\vskip6pt
\begin{tcolorbox}[quotebox, width=11cm]
\scriptsize This is real API usage since March. The \$20/mo subscription covers most people --- API pricing is for heavy or automated use.
\end{tcolorbox}
\end{frame}
...
\scriptsize\bfseries These slides were also made with AI.
\end{tcolorbox}
\end{frame}
...
%% ====== SECTION DIVIDER: APPLICATION — VOICE FILE ======
{
\setbeamertemplate{footline}{}
...
\begin{tikzpicture}[remember picture, overlay]
  ...
  \node[text=UVMGold, font=\normalsize]
    at ([yshift=-0.4cm]current page.center) {Building Your Writing Voice};
  \draw[UVMGold, line width=0.8pt] ([xshift=-2.5cm, yshift=0cm]current page.center) -- ([xshift=2.5cm, yshift=0cm]current page.center);
\end{tikzpicture}
\end{frame}
}

%% ====== APPLICATION 1: VOICE FILE OVERVIEW ======
\begin{frame}{Application 1: Building Your Writing Voice}
\begin{columns}[T]
\begin{column}{0.50\textwidth}
\begin{tcolorbox}[keybox, title={\faPen\quad The Task}]
\scriptsize
Create a ``voice file'' that captures how you write --- then use it to draft emails, letters, and feedback that sound like you.
\vskip3pt
\textbf{Input:} 3--5 samples of your real writing\\
\textbf{Output:} A reusable \texttt{.md} file describing your style\\
\textbf{Time:} $\sim$5 minutes to create, seconds to use
\end{tcolorbox}
\end{column}
\begin{column}{0.44\textwidth}
\vskip2pt
...
\textbf{Why this demo?}
\begin{itemize}\setlength{\itemsep}{0pt}\tiny
  \item No installation needed --- works in any AI tool
  \item Immediately useful for email and writing
  \item Shows the power of context over prompting
  \item You can start doing this today
\end{itemize}
\end{tcolorbox}
\vskip3pt
{\scriptsize\color{CoolGray} \faPlay\; We'll walk through the process live.}
\end{column}
\end{columns}
\end{frame}
...
\begin{tcolorbox}[colback=PaleSage, colframe=CoolGray, boxrule=0.5pt, arc=3pt, left=3pt, right=3pt, top=2pt, bottom=2pt, title={\tiny ChatGPT (not logged in)}, fonttitle=\bfseries\tiny, coltitle=DeepCharcoal, colbacktitle=PaleSage!80]
\centering
\includegraphics[width=\textwidth]{voice-chatgpt-not-logged-in.png}
\end{tcolorbox}
\end{column}
\begin{column}{0.04\textwidth}
\centering\vskip1cm
{\color{UVMGold}\faArrowRight}
\end{column}
\begin{column}{0.28\textwidth}
\begin{tcolorbox}[colback=PaleSage, colframe=CoolGray, boxrule=0.5pt, arc=3pt, left=3pt, right=3pt, top=2pt, bottom=2pt, title={\tiny ChatGPT (logged in)}, fonttitle=\bfseries\tiny, coltitle=DeepCharcoal, colbacktitle=PaleSage!80]
\centering
\includegraphics[width=\textwidth]{voice-chatgpt-logged-in.png}
\end{tcolorbox}
\end{column}
\begin{column}{0.04\textwidth}
\centering\vskip1cm
...
\end{column}
\begin{column}{0.28\textwidth}
\begin{tcolorbox}[colback=LightGold, colframe=UVMGold, boxrule=0.5pt, arc=3pt, left=3pt, right=3pt, top=2pt, bottom=2pt, title={\tiny Claude + voice file}, fonttitle=\bfseries\tiny, coltitle=DeepCharcoal, colbacktitle=LightGold!80]
\centering
\includegraphics[width=\textwidth]{voice-claude-with-voice.png}
\end{tcolorbox}
\end{column}
\end{columns}
\vskip6pt
...
{\tiny $\bullet$ Tone: direct, warm, slightly informal\\
...
\vskip3pt
\begin{tcolorbox}[quotebox, top=3pt, bottom=3pt]
\scriptsize Output looks like \textbf{you wrote it} --- not like a chatbot wrote it. That's the point.
\end{tcolorbox}
\end{column}
\end{columns}
\end{frame}
